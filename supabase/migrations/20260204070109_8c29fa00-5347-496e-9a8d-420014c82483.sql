-- Update get_enhanced_leaderboard to support both legacy level_id and new category_id
-- The function now handles:
-- 1. If p_level_id matches a legacy level, use sections → levels path
-- 2. If p_level_id matches a category (first layer), find its descendant categories and filter questions by category_id
...
BEGIN
  ...
  question_counts AS (
    ...
    WHERE q.parent_id IS NULL
      AND (
        CASE 
          WHEN v_is_category THEN q.category_id IN (SELECT id FROM category_tree)
          WHEN v_is_legacy_level THEN sec.level_id = p_level_id
          ELSE true
        END
      )
    GROUP BY group_id
