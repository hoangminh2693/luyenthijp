-- Fix get_enhanced_leaderboard to handle hybrid category/section data
-- When a category is passed, also include questions from the corresponding legacy level
...
BEGIN
  ...
  WITH RECURSIVE category_tree AS (
    -- Base case: the selected category and its children
  ...
  -- Get root categories (first layer) for grouping
  root_categories AS (
    SELECT 
      c.id,
      c.name,
  ...
  question_counts AS (
      ...
      CASE 
        ...
        WHEN v_is_category OR (NOT v_is_legacy_level AND p_level_id IS NULL) THEN
          ...
          COALESCE(
            ...
            -- Fallback: map section's level to root category
            (SELECT rc.id FROM root_categories rc
    ...
    WHERE q.parent_id IS NULL
      AND (
        CASE 
          -- For category filter: include both category matches AND legacy section matches
  ...
  SELECT 
    ...
    COALESCE(
      ...
      -- For filtered view, get name from category or level
...
COMMENT ON FUNCTION public.get_enhanced_leaderboard IS 'Enhanced leaderboard function that supports both legacy level system and new category system. When a category ID is passed, it also includes data from the corresponding legacy level (matched by slug/name) to ensure complete results during the transition period.';